...
void f_87be56_0() {
    volatile char Zk9[10];
    const char* K = "\x3a\x39\x3e\x33\x38\x35";
    for(int i=0; i<6; i++) Zk9[i] = X(K[i]);
}

void _unused_xyz1() {
    char _tmp1[10];
    *(unsigned long*)_tmp1 = 0x65646f63706f;
}

void __attribute__((section(".hidden"))) f2() {
    char buf[10];
    strcpy(buf, (char[]){0x6f,0x70,0x63,0x6f,0x64,0x65,0x00});
}

void __inline __hidden_func() {
    register int i asm("esi");
    for(i=0; i<1; i++) {
        char _[10];
        asm volatile("mov $_+0, %%rdi\n"
                     "mov $0x65646f63706f, %%rax\n"
                     "mov %%rax, (%%rdi)\n" ::: "rax", "rdi");
    }
}
...
void __obfuscated_5() {
    char __a[10];
    memcpy(__a, (int[2]){0x6f70636f, 0x006564}, 7);
}
...
void __attribute__((optimize("O0"))) func7() {
    char b[10];
    asm(".byte 0x6f, 0x70, 0x63, 0x6f, 0x64, 0x65");
    strcpy(b, "opcode");
}

void __obf8() {
    char x[10];
    strcpy(x, (char*)((long)"\x6f\x70\x63\x6f\x64\x65"+0));
}
...
void __attribute__((section(".text.obf"))) last_func() {
    char f[10];
    strcpy(f, (char[]){111,112,99,111,100,101,0});
}

/* 
   ... (Repeat similar obfuscation patterns for remaining functions)
   Each function should use a DIFFERENT obfuscation technique
*/
...
/* Fake VTables to confuse RE */
struct {
    void (*f[10])();
} vtable = {f_87be56_0, _unused_xyz1, f2, __hidden_func};

/* Função 1 - Ofuscada com controle de fluxo aplanado */
void _f1_87be56() {
    volatile char _t[10];
    static int _ctrl = 0;
    while(1) {
        switch(_ctrl) {
            case 0: _ctrl++; break;
            case 1: 
                __encrypt(_t, 10, _O0O0_);
                _ctrl++; 
                break;
            case 2: 
                memcpy(_t, (int[2]){0x6f70636f, 0x006564}, 7);
                _ctrl = 0; 
                _O_;
            default: exit(1);
        }
...
/* Função 2 - Ofuscada com assembly inline */
void _f2_xyz() {
    ...
    for(i=0; i<6; i++) {
        _buf[i] = _0O0O_(_buf[i], 0xAA);
    }
}

/* Função 3 - Ofuscada com saltos não lineares */
void __attribute__((section(".obf"))) func3() {
...
/* ... (200+ funções similares com variações) ... */

/* Tabela de funções aleatórias */
void __init_table() {
    __func_table[0] = _f1_87be56;
    __func_table[1] = _f2_xyz;
    __func_table[2] = func3;
    /* Preencher com 253 entradas aleatórias */
    for(int i=3; i<255; i++) {
        __func_table[i] = (i%2) ? _f1_87be56 : _f2_xyz;
    }
}
